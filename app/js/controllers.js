...
function MyCtrl1($scope, $http, $resource, $timeout, sharedService, twitter, configurationService) {
    ...
    $scope.previousTweets = function () {
        if (this.tweets.statuses) {
            var maxId = $scope.tweets.statuses.reduce(function (s1, s2) {
                return s1.id_str > s2.id_str ? s1.id_str : s2.id_str
            })
            $scope.tweets = twitter.paginate({since_id: maxId, q: $scope.searchQuery})
        }
    }
    $scope.nextTweets = function () {
        if (this.tweets.statuses) {
            var minId = $scope.tweets.statuses.reduce(function (s1, s2) {
                return s1.id_str < s2.id_str ? s1.id_str : s2.id_str
            })
            $scope.tweets = twitter.paginate({max_id: minId, q: $scope.searchQuery})
        }
    }
